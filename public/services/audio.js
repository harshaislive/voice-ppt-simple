export class StreamAudioPlayer {
    constructor() {
        ...
        this._isPaused = false;  // Track paused state internally
    }
    ...
    // Pause - stops all playback and ignores new chunks until resumed
    pause() {
        this._isPaused = true;
    ...
    // Resume - allows new chunks to be processed
    resume() {
    ...
    playChunk(pcmBase64, sampleRate, channels) {
        // REJECT all chunks while paused - this is the key fix
        if (this._isPaused) {
            return;
        }
        ...
        // Start buffering mode on first chunk
        ...
        // Buffer chunks until flush timer fires
        if (this.isBuffering) {
    ...
    _flushQueue() {
        ...
        while (this.chunkQueue.length > 0) {
            // Check pause state before each chunk
            if (this._isPaused) {
                return;
            }
